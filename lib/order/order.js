...
const { get_client_info } = require('./functions');
...
class Order{
  ...
  async get_orders_by_client_id(client_id){
    try {
      const orders = await OrderSchema.find({client_id});
  ...
  async get_all_active_orders_by_client(client_id){
    try {
      const orders = await OrderSchema.find({client_id, deleted: false});
      return http_response(200, 'Order info', {orders});
    } catch (error) {
      return http_response(500, 'Internal server error in order', {error});
    }
  ...
  async create({cad_number, ...body}, client_id){
    try {
      if(cad_number){
      ...
      const order_to_create = new OrderSchema({...body, client_id});
