...
class Order{
  async get_all_orders(){
    try {
      const orders = await OrderSchema.find().sort({createdAt: -1}).limit(100).lean(true);
  ...
  async get_orders_by_clientName(clientName){
    try {
      const orders = await OrderSchema.find({clientName}).sort({createdAt: -1}).limit(50);
  ...
  async edit(body, id){
    try {
      ...
      if(body.deliveredDate) body.deliveredDate = new Date(body.deliveredDate);
  ...
  async delete(id){
    try {
      const order_deleted = await OrderSchema.findById(id);
      if(order_deleted?.deliveredDate) return http_response(400, `The order can not be deleted because was delivered`, {error: true});
      return http_response(200, `Order deleted`, {invoice});
    } catch (error) {
      return http_response(500, 'Internal error in order', {error: true});
    }
  };
