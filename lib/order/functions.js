...
const generate_jobId = async (clientName) => {
  const orders_by_client = await OrderSchema.find({clientName}).sort({createdAt: 1});
...
const mark_invoice_in_orders = async (orders, invoice) => {
  return await Promise.all(
    orders.map( async(order) => {
      return await OrderSchema.findByIdAndUpdate(order._id, {invoice});
    })
  );
};
