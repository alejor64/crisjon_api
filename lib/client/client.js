...
class Client{
  ...
  async edit(info_to_update, client_id){
    try {
      ...
      const client_updated = await ClientSchema.findByIdAndUpdate(client_id, info_to_update, {new: true}).lean(true);
      return http_response(200, `Client ${client_updated.name} updated successfully`, {cliente: client_updated});
    } catch (error) {
      return http_response(500, 'Internal server error', {error});
    }
  };
