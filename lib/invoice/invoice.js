...
const { mark_orders_as_payed } = require('../order/functions');
...
class Invoice {
  ...
  async edit(id, body) {
    try {
      ...
      const outstandingBalance = (body.totalPrice - body.pricePayed) || 0;
      body.outstandingBalance = outstandingBalance;
      const invoice = await InvoiceSchema.findByIdAndUpdate(id, body, {new: true});
      await ClientSchema.findOneAndUpdate({name: body.clientName}, {outstandingBalance})
      if(body?.payed) await mark_orders_as_payed(body.orders, body.paymentType, body.checkNumber)
  ...
  async deleteById(id){
    try {
      ...
      await Promise.all(invoice_deleted.ordersPayed.map(async(order) => await OrderSchema.findByIdAndUpdate(order._id, {payed: false, paymentDate: ""})))
